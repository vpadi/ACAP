   ...
   */
   ...
   for(int i = 1; i <= (my_height/p_size)+1; ++i){
      for(int j = 1; j <= my_width; ++j){
         /* En vez de hacer un tercer bucle anidado, realizamos un desenrollado de bucle */
         R_cop[(i*my_width)+j] += R[((i-1)*my_width)+(j-1)] * conv[0][0] + R[((i)*my_width)+(j-1)] * conv[1][0] + R[((i+1)*my_width)+(j-1)] * conv[2][0];
         G_cop[(i*my_width)+j] += G[((i-1)*my_width)+(j-1)] * conv[0][0] + G[((i)*my_width)+(j-1)] * conv[1][0] + G[((i+1)*my_width)+(j-1)] * conv[2][0];
         B_cop[(i*my_width)+j] += B[((i-1)*my_width)+(j-1)] * conv[0][0] + B[((i)*my_width)+(j-1)] * conv[1][0] + B[((i+1)*my_width)+(j-1)] * conv[2][0];

         R_cop[(i*my_width)+j] += R[((i-1)*my_width)+(j)] * conv[0][1] + R[((i)*my_width)+(j)] * conv[1][1] + R[((i+1)*my_width)+(j)] * conv[2][1];
         G_cop[(i*my_width)+j] += G[((i-1)*my_width)+(j)] * conv[0][1] + G[((i)*my_width)+(j)] * conv[1][1] + G[((i+1)*my_width)+(j)] * conv[2][1];
         B_cop[(i*my_width)+j] += B[((i-1)*my_width)+(j)] * conv[0][1] + B[((i)*my_width)+(j)] * conv[1][1] + B[((i+1)*my_width)+(j)] * conv[2][1];

         R_cop[(i*my_width)+j] += R[((i-1)*my_width)+(j+1)] * conv[0][2] + R[((i)*my_width)+(j+1)] * conv[1][2] + R[((i+1)*my_width)+(j+1)] * conv[2][2];
         G_cop[(i*my_width)+j] += G[((i-1)*my_width)+(j+1)] * conv[0][2] + G[((i)*my_width)+(j+1)] * conv[1][2] + G[((i+1)*my_width)+(j+1)] * conv[2][2];
         B_cop[(i*my_width)+j] += B[((i-1)*my_width)+(j+1)] * conv[0][2] + B[((i)*my_width)+(j+1)] * conv[1][2] + B[((i+1)*my_width)+(j+1)] * conv[2][2];

         /* Aplicamos el divisor del color */
